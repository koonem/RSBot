...
public class UltimateUser extends Node {
	...
	public void execute() {
		...
		Actionbar.Ability[] ultimates = getUltimateCount() > 0 ? new Actionbar.Ability[getUltimateCount()] : new Actionbar.Ability[0];
		byte j = 0x0;
		for(int i = 0; i < 12; i++)
		{
			if(Actionbar.getAbilityAt(i) != null && Actionbar.getAbilityAt(i).getAbilityType() == Actionbar.AbiltiyType.ULTIMATE)
			{
	...
	private byte getUltimateCount()
	{
		...
		for(byte i = 0; i < 12; i++)
		{
			j = Actionbar.getAbilityAt(i).getAbilityType() == Actionbar.AbiltiyType.ULTIMATE ? j++ : j ;
		}
		return j;
	}
