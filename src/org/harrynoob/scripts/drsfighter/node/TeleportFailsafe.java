...
public class TeleportFailsafe extends Node {
	...
	public void execute() {
		...
		WidgetChild w1;
		if ((w1 = Widgets.get(ABILITY_BOOK_WIDGET_ID,
				ABILITY_BOOK_CHILD_WIDGET_ID)) != null && w1.validate()) {
			...
			WidgetChild w4 = w1
					.getChild(ABILITY_BOOK_MAGIC_TAB_HOME_TELEPORT_WIDGET_ID);
			if (w4 != null && w4.validate()) {
				w4.interact("Cast");
			}
		}
		WidgetChild lodestones = Widgets.get(LODESTONE_WIDGET_ID,
				VARROCK_LODESTONE_WIDGET_ID);
